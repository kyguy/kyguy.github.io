...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This beauty was provided by Alex from StackOverflow
% http://tex.stackexchange.com/questions/188183/latex-under-construction-in-work-or-work-in-progress-symbols/188186#188186
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\centerline{\sc \large k-glyphs}
\centerline{\sc Technology Explantions for the Layperson }
%\centerline{\url{https://github.com/jboss-openshift/ce-testsuite.git/}}

\section{}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Cloud Enablement Emblem
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}
\begin{tikzpicture}
 
  %Cloud
  \tikzstyle{teal_circle}=[circle, fill=teal!30!white , minimum width=2cm]
  \tikzstyle{gray_circle}=[circle, fill=gray!30!white, minimum width=2cm]
  
  \path ( 1,1) node     [teal_circle] {}
        ( 0,0) node     [teal_circle] {}
        (-1,1) node     [teal_circle] {}
        (-2,0) node     [teal_circle] {}
        (2,0) node      [teal_circle] {}
        (-1,-1) node    [teal_circle] {}
        ( 1,-1) node    [teal_circle] {}
        ( 0, 1.2) node  [teal_circle] {}
        ( 0,-1.2) node  [teal_circle] {};
 
  %CE Abbreviation
  \tikzstyle{end}=[circle,draw=black!50,fill=black!20,thick,
  inner sep=0pt,minimum size=2mm]
  \tikzstyle{v_end}=[rectangle,draw=black!50,fill=black!50,thick,
  inner sep=0pt,minimum height=2mm]
  \tikzstyle{h_end}=[rectangle,draw=black!50,fill=black!50,thick,
  inner sep=0pt,minimum width=2mm]
  \tikzstyle{blank}=[inner sep=0pt,minimum width=2mm]
  \tikzstyle{c_end}=[rectangle,draw=black!50,fill=black!50,thick,
  inner sep=0pt,minimum height=2mm, minimum width=2mm ]
  \tikzstyle{pipe}=[cylinder, draw=gray!50, line width=5pt]
  
  \node[v_end] (c_top)  at (-0.5,1)        {};
  \node[blank] (c_mid)  [below of = c_top] {};
  \node[c_end] (c_left) [left of  = c_mid] {};
  \node[v_end] (c_bot)  [below of = c_mid] {};

  \draw[pipe]  (c_top)  to [bend right = 45] (c_left);
  \draw[pipe]  (c_left) to [bend right = 45] (c_bot);

  \node[c_end] (e_top)  [right of= c_top] {};
  \node[c_end] (e_mid)  [right of= c_mid] {};
  \node[c_end] (e_bot)  [right of= c_bot] {};

  \node[v_end] (e_top_right)  [right of= e_top] {};
  \node[v_end] (e_mid_right)  [right of= e_mid] {};
  \node[v_end] (e_bot_right)  [right of= e_bot] {};

  \draw[pipe]  (e_top) -- (e_mid);
  \draw[pipe]  (e_mid) -- (e_bot);
  \draw[pipe]  (e_top) -- (e_top_right);
  \draw[pipe]  (e_mid) -- (e_mid_right);
  \draw[pipe]  (e_bot) -- (e_bot_right);
\end{tikzpicture}
\end{figure}

\subsection{Cloud Enablement Middleware}
\begin{itemize}  
  \item \href{https://kyguy.github.io/src/containers/containers.html}{Containers} \ucmark
  \item \href{https://kyguy.github.io/src/openshift/openshift.html}{OpenShift} \ucmark
  \item \href{https://kyguy.github.io/src/hacking_openshift/hacking_openshift.html}{Hacking OpenShift} \ucmark
  %\item \href{http://kyguy.github.io/src/testsuite/testsuite.html}{Cloud Enablement Testsuite(Architecture)} \ucmark
\end{itemize}
\ucmark Currently under development
