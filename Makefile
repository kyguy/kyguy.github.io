...
directories = home testsuite hacking_openshift openshift
...
define view
view:: ; trap 'kill -9  $(PID)' TERM INT && cd $(SRC)/$(1) &&  rm -f $(JUNK); pdflatex -shell-escape $(1).tex; htlatex $(1).tex "$(BASE)/$(SRC)/gen.cfg,xhtml"; cp * $(BASE)/$(OUT)/$(1)/; firefox ../../$(OUT)/$(1)/$(1).html &
endef
